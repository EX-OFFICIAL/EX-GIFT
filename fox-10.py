...
class Main:
	...
	def fbtua(self):
		...
		try:
			...
			os.system('https://www.facebook.com/meri.jan.ummmah')
			os.system('clear')
			print(logo) 
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s\033[92;1m[🌼ADRIYAN🌼] : %s/%s -> \033[0;92m [ OK:%s ]- \033[0;91m[CP:%s ]"%(B,self.loop, len(self.id), len(self.cp), len(self.ok))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r  \033[1;32m   [ADRIYAN] %s | %s\033[1;32m         "%(uid, pw))
